...
const raffleTabButtons = document.querySelectorAll('.raffles-tab-button');
const raffleContents = document.querySelectorAll('.raffles-content');
const raffleTabHighlight = document.querySelector('.raffles-tab-highlight');

function updateRaffleHighlight(activeButton) {
    const buttonRect = activeButton.getBoundingClientRect();
    ...
    raffleTabHighlight.style.left = `${left}px`;
    raffleTabHighlight.style.width = `${width}px`;
}

function switchRaffleTab(tabName) {
    // Update active tab button
    raffleTabButtons.forEach(button => {
        button.classList.toggle('active', button.dataset.raffleTab === tabName);
    });
    
    // Update active content
    raffleContents.forEach(content => {
        content.classList.toggle('active', content.dataset.raffleContent === tabName);
    });
    
    // Update highlight position
    const activeButton = document.querySelector(`.raffles-tab-button[data-raffle-tab="${tabName}"]`);
    if (activeButton) {
        updateRaffleHighlight(activeButton);
    }
}

// Initialize raffle tabs
if (raffleTabButtons.length) {
    const activeRaffleTab = document.querySelector('.raffles-tab-button.active');
    if (activeRaffleTab) {
        updateRaffleHighlight(activeRaffleTab);
        switchRaffleTab(activeRaffleTab.dataset.raffleTab);
    }
